...
import { getAllRefundsList, updateRefundRequest } from "../../redux/slices/refundSlice";
import { useDispatch } from "react-redux";
import Select from "react-select";
const CouponTable = ({ data, title }) => {
...
// Filter out inactive from dropdown options, but keep it for display
  ...
  const handleStatusChange = (id,  newStatus) => {
    dispatch(updateRefundRequest({ id: id, status: newStatus }))
      .then(() => {
        dispatch(getAllRefundsList());
      });
  };
            ...
            {currentItems.length > 0 ? (
              currentItems.map((item, index) => (
                <tr key={item?.id}>
                  ...
                  {visibleColumns.phone && (
                    <td className="px-6 py-4 capitalize">{item?.user_phone}</td>
                  )}
                  {visibleColumns.total && (
                    <td className="px-6 py-4 ">{item?.total}</td>
                  )}
                  {visibleColumns.reason && (
                    <td className="px-6 py-4 capitalize">{item?.reason}</td>
                  )}
                  {visibleColumns.store_name && (
                    <td className="px-6 py-4 capitalize">{item?.store_name}</td>
                  )}
                  {visibleColumns.store_email && (
                    <td className="px-6 py-4 ">{item?.store_email}</td>
                  )}
                  {visibleColumns.store_phone && (
                    <td className="px-6 py-4 ">
                      { item?.store_phone ? item?.store_phone : '-'}
                    </td>
                  )}
                  {visibleColumns.created_at && (
                  <td className="px-6 py-4 capitalize">
                    ...
                    <td className="px-2 py-2 whitespace-nowrap">
                      <div className="relative inline-block">
                        <Select
                          ...
                          onChange={(selectedOption) => handleStatusChange(item?.id, selectedOption.value)}
                          isSearchable={false}
                          className="text-sm"
