...
const NotificationTable = ({ data, title, onEdit, onDelete }) => {
  // Original state
  ...
  const [sortField, setSortField] = useState("notification_type");
        ...
        <table className="w-full" ref={tableRef}>
          <thead className="bg-gray-50 border-b border-gray-200">
            <tr>
              ...
              {visibleColumns.notification_type && (
                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                  Notification Type
                </th>
              )}
            ...
            {currentItems.length > 0 ? (
              currentItems.map((item, index) => (
                <tr key={item?.id}>
                  ...
                  {visibleColumns.notification_type && (
                    <td className="px-6 py-4 capitalize">{item?.notification_type}</td>

                    // <td className="px-6 py-4">
                    //   {item.image ? (
                    //     <a
                    //       style={{ cursor: "pointer" }}
                    //       href={`${baseUrl}/images/${item.image}`}
                    //       download
                    //       target="_blank"
                    ...
                    //   {item?.image ? (
                    //       <div className="w-10 h-10 sm:w-13 sm:h-13  rounded-xl overflow-hidden">
                    //         <img
