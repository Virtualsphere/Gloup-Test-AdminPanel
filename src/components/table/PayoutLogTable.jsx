...
const PayoutLogTable = ({ data }) => {
  ...
  const handleDownloadCSV = () => {
    ...
    link.setAttribute("download", `payoutlog_data.csv`);
        ...
        <table className="w-full" ref={tableRef}>
          <thead className="bg-gray-50 border-b border-gray-200">
            <tr>
              ...
              {visibleColumns.amount_added && (
                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                  Amount Added
                </th>
              )}
              {visibleColumns.balance_before && (
                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                  Balance Before
                </th>
              )}
            ...
            {currentItems.length > 0 ? (
              currentItems.map((item, index) => (
                <tr key={item?.id}>
                  ...
                  {visibleColumns.amount_added && (
                    <td className="px-6 py-4 ">{item?.amount_added}</td>
                  )}
                  {visibleColumns.balance_before && (
                    <td className="px-6 py-4 ">{item?.balance_before}</td>
                  )}
                  {visibleColumns.balance_after && (
                    <td className="px-6 py-4 ">{item?.balance_after}</td>
                  )}
                  {visibleColumns.updated_at && (
