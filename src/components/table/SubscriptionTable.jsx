...
// import {
//   getAllSubscriptionsList, updateSubscriptionStatus
// } from "../../redux/slices/subscriptionSlice";
// import { useDispatch } from "react-redux";
// import Select from "react-select";

// const SubscriptionTable = ({ data, title, onEdit }) => {
...
//     dispatch(updateSubscriptionStatus({ id: id, status: newStatus }))
//       .then(() => {
...
//                   )}
//                   {visibleColumns.days && (
//                     <td className="px-6 py-4 capitalize">{item?.days}</td>
//                   )}
//                   {visibleColumns.price && (
...
const SubscriptionTable = ({ data, title, onEdit }) => {
  ...
  return (
    <div>
      {/* 🔹 Tabs Navigation */}
      <div className="flex space-x-3 border-b border-gray-200 mb-4 px-4">
        {tabs.map((tab) => (
          <button
            key={tab.value}
            onClick={() => setActiveTab(tab.value)}
            className={`px-4 cursor-pointer py-2 text-sm font-medium rounded-t-md transition-all duration-200 ${
              activeTab === tab.value
                ? "bg-black text-white"
