...
const Refund = ({title }) => {
  ...
  const allRefundsList = useSelector((state) => state.allRefunds.allRefundsList);
  const loading = useSelector((state) => state.allRefunds.loading); // fixed key
  const error = useSelector((state) => state.allRefunds.error);     // fixed key
      ...
      <div>
        ...
        {error ? (
          <div className="text-red-600 bg-red-50 border border-red-200 p-4 rounded-md mb-4">
            ⚠️ Failed to load Refunds: {error}
          </div>
         ) : loading ? (
